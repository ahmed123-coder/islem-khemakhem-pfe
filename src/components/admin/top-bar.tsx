        ...
        <DropdownMenu>
           <DropdownMenuTrigger asChild>
             <Button variant="ghost" size="icon" className="relative group rounded-2xl hover:bg-slate-100/80 transition-all active:scale-95">
                ...
                {unreadCount > 0 && (
                  <span className="absolute top-2.5 right-2.5 w-2.5 h-2.5 bg-blue-600 rounded-full border-2 border-white pointer-events-none animate-pulse" />
                )}
             </Button>
           </DropdownMenuTrigger>
           <DropdownMenuContent align="end" className="w-80 p-0 rounded-3xl shadow-2xl border-slate-100 overflow-hidden bg-white/95 backdrop-blur-xl">
              <div className="p-5 bg-slate-50/50 flex items-center justify-between border-b border-slate-100">
                 ...
                 {unreadCount > 0 && (
                   <Button 
                     ...
                     className="h-7 text-[9px] font-bold uppercase text-blue-600 hover:bg-blue-50 rounded-lg px-2"
                   >
                     Clear all
                   </Button>
                 )}
              ...
              <div className="p-4 border-t border-slate-100 bg-slate-50/30 text-center">
                 <Link href="/admin" className="text-[10px] font-bold uppercase tracking-widest text-slate-400 hover:text-blue-600 transition-colors">
                    View Recent Logs
                 </Link>
              </div>
           </DropdownMenuContent>
