...
export default function Navbar() {
  ...
  useEffect(() => {
    fetch('/api/content/navbar')
      ...
      .catch(() => setContent({ logo: 'DSL Consulting', links: [] }))
  }, [])
        ...
        <div className="flex justify-between h-16 items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {content.links.map((link, i) => (
              ...
              >
                {link.href === '/services' ? "Domaines d'intervention" : link.label}
              </Link>
            ))}
          </div>
      ...
      {isOpen && (
        <div className="md:hidden border-t">
          <div className="px-4 pt-4 pb-6 space-y-4 bg-white">
            ...
            <Link href="/login" className="flex items-center gap-2 text-[#1B3F7A] font-medium py-2">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-[#7AB648]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" />
              </svg>
              Connexion
