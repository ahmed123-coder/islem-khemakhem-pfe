...
export default function ClientReservations() {
  const t = useTranslations('clientPage.reservations')
  ...
  if (loading) return <div className="flex items-center justify-center min-h-screen">{t('noReservations')}</div>
    ...
    <div className="min-h-screen bg-gray-50/50 pb-20">
      <div className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex items-center justify-between mb-12">
           <h1 className="text-4xl font-serif font-black text-gray-900">{t('title')}</h1>
          ...
          <div className="flex flex-col xl:flex-row gap-10 items-start">
            ...
            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-6 gap-x-10">
               <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('clientId')}</label>
               ...
               <div className="space-y-2 lg:col-span-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('clientName')}</label>
                  <div className="bg-amber-100/40 border border-amber-200/50 rounded-xl px-4 py-3 font-bold text-gray-700 flex items-center text-sm">
                    {currentUser?.name || t('loginRequired')}
                  </div>
               </div>

               <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('appointmentDate')}</label>
                  <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 font-bold text-blue-600 flex items-center text-sm shadow-sm">
                    {latestReservation ? new Date(latestReservation.startTime).toLocaleDateString() : '-- / -- / ----'}
                  </div>
               </div>

               <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('startTime')}</label>
                  <div className="bg-white border border-gray-200 rounded-xl px-4 py-3 font-bold text-gray-700 flex items-center text-sm shadow-sm">
                    {latestReservation ? new Date(latestReservation.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-- : --'}
                  </div>
               </div>

               <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('appointmentStatus')}</label>
               ...
               <div className="space-y-2 lg:col-span-3">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] ml-1">{t('observation')}</label>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl shadow-blue-900/5 border border-gray-100 overflow-hidden">
          {reservations.length === 0 ? (
            <div className="p-20 text-center">
              ...
              <p className="text-gray-400 font-bold uppercase tracking-widest text-xs mb-6">{t('noReservations')}</p>
              <a href="/solutions" className="inline-flex items-center gap-2 bg-[#2B5A8E] text-white px-8 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg hover:shadow-2xl hover:-translate-y-1 transition-all">
                {t('discoverSolutions')}
              </a>
            </div>
          ) : (
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('phase')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('expertCol')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('dateTime')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('state')}</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">{t('directLink')}</th>
                </tr>
              </thead>
                  ...
                  <tr key={reservation.id}>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-bold text-gray-900">{reservation.sessionLabel || t('consultation')}</div>
                    ...
                    <td className="px-6 py-4">
                      <div className="font-bold">{new Date(reservation.startTime).toLocaleDateString()}</div>
                      <div className="text-[10px] text-gray-400 font-bold uppercase">
                        {new Date(reservation.startTime).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(new Date(reservation.endTime).getTime() - 15 * 60 * 1000).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </div>
                    </td>
                    <td className="px-6 py-4">
