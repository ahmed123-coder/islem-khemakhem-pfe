...
export default function ConsultantMissionsPage() {
  const [missions, setMissions] = useState<Mission[]>([])
  const [filter, setFilter] = useState<string>('ALL')
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    fetch('/api/consultant/missions')
      .then(r => r.json())
      .then(data => setMissions(data))
      .finally(() => setLoading(false))
  }, [])

  const filteredMissions = Array.isArray(missions) ? missions.filter(m => 
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">Manage and track all your consulting missions</p>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-4 text-sm text-gray-500">
                    <span>Client: {mission.client.name || mission.client.email}</span>
                    <span>•</span>
